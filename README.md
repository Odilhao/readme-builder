...
The resolved data is passed to templates as a `Data` struct with the following fields:

```go
type Data struct {
    User      User                  // GitHub user/org being rendered
    GitHub    GitHub                // Fetched GitHub data
    Feeds     map[string][]FeedItem // Fetched RSS feeds by name
    Now       time.Time             // Fetch timestamp (UTC)
}
...
type GitHub struct {
    Contributions []Contribution  // Derived from public events
    Repos         []Repo          // Owned repositories
    Forks         []Repo          // Forked repositories
    PullRequests  []PullRequest   // Pull requests
    Stars         []Star          // Starred repositories (not yet implemented; always empty)
    Releases      []Release       // Releases (not yet implemented; always empty)
    Followers     []Follower      // Followers (not yet implemented; always empty)
    Gists         []Gist          // Public gists (not yet implemented; always empty)
}

type Contribution struct {
    Repo   string  // Repository name (owner/repo)
    Events int     // Event count from public timeline (~90 days, max 300 events)
}

type Repo struct {
    Name        string    // Repository name (bare name, not owner/repo)
    FullName    string    // Full name (owner/repo)
    Description string    // Repository description
    URL         string    // Repository URL
    Language    string    // Primary programming language
    Stars       int       // Star count
    PushedAt    time.Time // Last push time (RFC 3339)
}
...
type PullRequest struct {
    ...
    MergedAt time.Time // Merge time (zero if not merged; RFC 3339)
}

type Star struct {
    Repo      Repo      // Starred repository
    StarredAt time.Time // When starred (RFC 3339)
}

type Release struct {
    Repo        string    // Repository name (owner/repo)
    TagName     string    // Git tag
    Name        string    // Release name (may differ from tag)
    URL         string    // Release URL
    PublishedAt time.Time // Publication time (RFC 3339)
}
...
type Gist struct {
    ...
    URL         string    // Gist URL
    CreatedAt   time.Time // Creation time (RFC 3339)
}

type FeedItem struct {
    Title       string    // Feed item title
    URL         string    // Link
    Published   time.Time // Publication time (RFC 3339)
    Description string    // Summary/description
}
```

All timestamps are in UTC and represented as RFC 3339 strings in JSON output.

## Template functions

Templates receive the data model above and have access to one custom function:
...
Output examples: `now`, `today`, `1d ago`, `3d ago`, `2w ago`, `1mo ago`, `2y ago`.

Special behavior:
- Both the input and reference time (`Now`) are flattened to midnight UTC before comparison.
- This ensures that hourly cron jobs produce stable output without drift.
- Empty/zero times return an empty string.
...
- [{{ .FullName }}]({{ .URL }}) – {{ .Description }}
  - Language: {{ .Language }} | Stars: {{ .Stars }} | Updated: {{ humanize .PushedAt }}
{{ end }}

## Recent contributions
{{ range .GitHub.Contributions }}
- [{{ .Repo }}](https://github.com/{{ .Repo }}) ({{ .Events }} events)
{{ end }}
